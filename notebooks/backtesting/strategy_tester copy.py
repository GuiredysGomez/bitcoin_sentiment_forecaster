...
class Backtesting:
    ...
    """
    def __init__(self, model_name, X_train, y_train, X_val, y_val, X_test, y_test, 
    ...
    def run(self):
        ...
        # Almacenar los mejores parámetros para reutilizarlos como punto de partida 
        ...
        for start in tqdm(range(0, n_test, self.window_size), desc="Walk-Forward Backtesting"):
            # --- 1. Re-optimización del Modelo ---
             ...
             # Solo es necesario para lightgbm
            ...
            if "lightgbm" in self.model_name.lower():
                objective_func = lambda trial: self._objective_lgbm(trial, X_train_current, y_train_current_m, 
                                                                    X_val_current, y_val_current_m, class_weights)
            else:
            ...
            if "lightgbm" in self.model_name.lower():
                ...
                lgb_train = lgb.Dataset(X_train_current, label=y_train_current_m, 
                                        weight=[class_weights.get(c, 1.0) for c in y_train_current_m])
                lgb_val = lgb.Dataset(X_val_current, label=y_val_current_m, 
                                      weight=[class_weights.get(c, 1.0) for c in y_val_current_m], reference=lgb_train)
            ...
            # --- Lógica de predicción según el modelo ---
            if "lightgbm" in self.model_name.lower():
                # Predecir las probabilidades para el bloque actual usando la mejor iteración.
                y_prob_block = model.predict(X_pred_block, num_iteration=model.best_iteration)
            else:
                raise ValueError(f"El model_name '{self.model_name}' no está soportado para la predicción.")

            # Convertir probabilidades a señales (-1, 0, 1)
            ...
            for i in range(start, end):
                ...
                if i not in backtest_df.index: continue # REVISAR, MALA PRACTICA.
